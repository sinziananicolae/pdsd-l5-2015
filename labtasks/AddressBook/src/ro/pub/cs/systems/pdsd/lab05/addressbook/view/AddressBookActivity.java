	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		switch (id) {
			...
			case R.id.action_update:
			    ...
			    String[] contactPhoneNumberParts = contactPhoneNumbers.replaceAll(" ", "").split("[:\n]");
			    if ((contactPhoneNumberParts.length % 2) == 0) {
			    	ArrayList<PhoneNumber> contactPhones = new ArrayList<PhoneNumber>();
			    	index = 0;
			    	while (index < contactPhoneNumberParts.length) {
			    		PhoneNumber contactPhoneNumber  = new PhoneNumber(contactPhoneNumberParts[index + 1], contactPhoneNumberParts[index]);
